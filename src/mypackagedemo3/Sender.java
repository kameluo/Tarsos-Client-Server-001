...
public class Sender {
	...
	public static void main(String[] args) throws LineUnavailableException, IOException {
			...
			//Receiving The Code Message from The Receiver (100-->means that the receiver"server" is logging in and waiting for receiving the messages from the sender"clients")
			byte[] buffer=new byte[100];
			DatagramPacket datagrampacket=new DatagramPacket(buffer,buffer.length);
			multicastsocket.receive(datagrampacket);
			String messagerecived=new String(buffer);
			System.out.println(messagerecived);
			char a=messagerecived.charAt(0);//getting the first character from the code sent by the receiver
