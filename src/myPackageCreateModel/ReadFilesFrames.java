...
public class ReadFilesFrames {
static ArrayList<float[]> arraylistMFCCforOneAudioFile=new ArrayList<float[]>();
static ArrayList<float[]> arraylist80frames=new ArrayList<float[]>();
...
FileOutputStream outputStream=new FileOutputStream("mel coef excel sheet average-80 frames not per audio files.xlsx");
...
public static void sendArray(float[] array) {
	...
	if(arraylistMFCCforOneAudioFile.size()>=79) {
		
		float[] arrayFinalAverage=averageAndClear();

		arraylist80frames.add(arrayFinalAverage);
...
Sheet sheet= workbook.createSheet(categoryName);
    int allength=arraylist80frames.size();
   
    for(int row=0;row<allength;row++){
    Row currentRow = sheet.createRow(row);
        for(int column = 0; column < 13; column++){
        Cell cell = currentRow.createCell(column);
            cell.setCellValue(arraylist80frames.get(row)[column]);
        }
    }
   
    arraylist80frames.clear();
}
 
 
public static float[] averageAndClear(){
int allength=arraylistMFCCforOneAudioFile.size();
float[] arrayFinalAverage=new float[13];
	for(int counter=0;counter<allength;counter++) {
		float[] arrayinsideal=arraylistMFCCforOneAudioFile.get(counter);
			for(int insidearray=0;insidearray<13;insidearray++) {
			arrayFinalAverage[insidearray]+=arrayinsideal[insidearray];
			}
		}
	for(int k=0;k<13;k++) {
	arrayFinalAverage[k]=arrayFinalAverage[k]/allength; 
	}
	arraylistMFCCforOneAudioFile.clear();
return arrayFinalAverage;
