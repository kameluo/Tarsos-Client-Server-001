...
public class Client {
	...
	public static void main(String[] args) throws IOException, LineUnavailableException {
		...
		char firstCharacterMessageReceivedMulticast=messageReceivedMulticast.charAt(0);//getting the first character from the code sent by the receiver
		InetAddress serverIP=datagramPacketmMlticastMessage.getAddress();//from the multicast message we will get the IP of The Server
		...
		byte [] b1=acknowledgementMessage.getBytes();//Transferring the Strings to Bytes
		DatagramPacket datagramPacketForUniCast=new DatagramPacket(b1,b1.length,serverIP,portunicast);//creating the packet
		...
		if(messagerecived=="200"){
		           ...
		           try {
					//first we will check"read" the last Word in the log file of the client Log File to not repeat it again,if it is different we will write it 
							...
							if(!currentState.equals(lastWord)){
								...
								filewritercase1.write(dateformat.format(currentdate)+currentState+"\r\n");
		  ...
		  //if The Client Receives "200" it means that the Server is Ready to get Receives the Sound States	
		}else if(messagerecived=="500"){
			
		}else if(messagerecived=="555"){
